import { useState, useCallback } from "react"
import { Badge, Stack } from "@chakra-ui/react"
import { useColorModeValue } from "@/components/ui/color-mode";
import { motion } from "framer-motion"

interface Point { x: number; y: number }

export const DraggablePointsSVG = () => {
  const grid = useColorModeValue("#e2e8f0", "#2d3748")
  const accent = "#3182ce"
  const accent2 = "#dd6b20"

  const [A, setA] = useState<Point>({ x: 40, y: 100 })
  const [B, setB] = useState<Point>({ x: 140, y: 100 })
  const [C, setC] = useState<Point>({ x: 90, y: 40 })
  const [D, setD] = useState<Point>({ x: 120, y: 60 })
  const [dragging, setDragging] = useState<string | null>(null)

  const handleMouseDown = (p: string) => () => setDragging(p)
  const handleMouseUp = () => setDragging(null)
  const handleMouseMove = useCallback((e: React.MouseEvent<SVGSVGElement>) => {
    if (!dragging) return
    const rect = e.currentTarget.getBoundingClientRect()
    const x = e.clientX - rect.left
    const y = e.clientY - rect.top
    const newPoint = { x, y }
    if (dragging === "A") setA(newPoint)
    if (dragging === "B") setB(newPoint)
    if (dragging === "C") setC(newPoint)
    if (dragging === "D") setD(newPoint)
  }, [dragging])

  // Vérification alignement A,B,C
  const areaABC = 0.5 * (A.x*(B.y-C.y) + B.x*(C.y-A.y) + C.x*(A.y-B.y))
  const aligned = Math.abs(areaABC) < 1e-2

  // Vérification cocyclicité (approximation simple)
  const dist = (P: Point, Q: Point) => Math.sqrt((P.x-Q.x)**2 + (P.y-Q.y)**2)
  const rAB = dist(A,B), rBC = dist(B,C), rCA = dist(C,A)
  const s = (rAB + rBC + rCA)/2
  const area = Math.sqrt(Math.max(s*(s-rAB)*(s-rBC)*(s-rCA),0))
  const R = (rAB*rBC*rCA)/(4*area || 1)
  const center = { x:(A.x+B.x+C.x)/3, y:(A.y+B.y+C.y)/3 }
  const rD = dist(D,center)
  const cocyclic = Math.abs(rD - R) < 10

  return (
    <Stack direction="column" gap={3} align="start">
      <svg
        viewBox="0 0 200 150"
        width="100%"
        height="200px"
        onMouseMove={handleMouseMove}
        onMouseUp={handleMouseUp}
        style={{ cursor: dragging ? "grabbing" : "default" }}
      >
        {/* Grille */}
        {Array.from({ length: 10 }).map((_, i) => (
          <line key={`gx${i}`} x1={i*20} y1={0} x2={i*20} y2={150} stroke={grid} strokeWidth={0.5}/>
        ))}
        {Array.from({ length: 8 }).map((_, i) => (
          <line key={`gy${i}`} x1={0} y1={i*20} x2={200} y2={i*20} stroke={grid} strokeWidth={0.5}/>
        ))}

        {/* Points */}
        {[{P:A,name:"A",color:accent},{P:B,name:"B",color:accent},{P:C,name:"C",color:accent2},{P:D,name:"D",color:accent2}]
          .map(({P,name,color})=>(
            <g key={name}>
              <motion.circle
                cx={P.x} cy={P.y} r={5} fill={color}
                onMouseDown={handleMouseDown(name)}
                animate={{ cx: P.x, cy: P.y }}
                transition={{ duration: 0.3, ease: "easeOut" }}
                whileHover={{ r: 7 }}
              />
              <text x={P.x+6} y={P.y-6} fontSize="10" fill="black">{name}</text>
            </g>
          ))}
      </svg>

      <Stack direction="row" gap={4} flexWrap="wrap">
        <Badge colorScheme={aligned ? "green":"gray"}>Alignés (A,B,C): {aligned?"Oui":"Non"}</Badge>
        <Badge colorScheme={cocyclic ? "green":"gray"}>Cocycliques (A,B,C,D): {cocyclic?"Oui":"Non"}</Badge>
      </Stack>
    </Stack>
  )
}
